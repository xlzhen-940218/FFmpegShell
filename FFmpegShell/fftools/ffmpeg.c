...
sigterm_handler(int sig)
{
    ...
    if(received_nb_signals > 3) {
        ret = _write(2/*STDERR_FILENO*/, "Received > 3 system signals, hard exiting\n",
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time, int64_t pts)
{
    ...
    // 获取 pts 的绝对值
   // 最安全的纯 C 语言方式，避免对无符号类型使用负号
    uint64_t effective_pts;
    if (pts < 0) {
        // 当 pts 为负时，将其转换为 int64_t 后取负号，然后转换为 uint64_t
        // 或者更直接地：(uint64_t)(0 - pts)
        effective_pts = (uint64_t)(-pts);
    }
    else {
        effective_pts = (uint64_t)pts;
    }
    us = effective_pts % AV_TIME_BASE;
    secs = (effective_pts / AV_TIME_BASE) % 60;
    mins = (effective_pts / AV_TIME_BASE / 60) % 60;
    hours = effective_pts / AV_TIME_BASE / 3600;
    hours_sign = (pts < 0) ? "-" : "";
